...
interface FDOperations {
 void openFD(int accNo, String name, double amount, int term) throws
InvalidAmountException;
 void viewFD(int accNo) throws AccountNotFoundException;
 void withdrawFD(int accNo, int yearsPassed) throws
AccountNotFoundException, PrematureWithdrawalException;
 void viewAllFDs();
}
class FDAccount {
 int accNo;
 ...
 boolean isActive;
 FDAccount(int accNo, String holderName, double principal, int term,
double interestRate) {
 this.accNo = accNo;
 ...
 return "FD Account No: " + accNo + ", Name: " + holderName + ",Principal: ₹" + principal + ", Term: " + term + " years, Rate: " + interestRate + "%,Active: " + isActive;
 }
}
class FDManagement implements FDOperations {
 ...
 private FDAccount findAccount(int accNo) throws
AccountNotFoundException {
 for (int i = 0; i < count; i++) {
 if (accounts[i].accNo == accNo && accounts[i].isActive) return
accounts[i];
 }
 throw new AccountNotFoundException("FD Account " + accNo + "not found!");
 }
 @Override
 public void openFD(int accNo, String name, double amount, int term)
...
acc.maturityAmount());
 ...
 @Override
 public void withdrawFD(int accNo, int yearsPassed) throws
AccountNotFoundException, PrematureWithdrawalException {
 FDAccount acc = findAccount(accNo);
